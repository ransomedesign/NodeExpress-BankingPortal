...
describe("Payment Feature", () => {
  ...
  before(() => {
    getStack =
      routeStack("/payment", "get") || routeStack("/services/payment", "get");
    ...
    postStack =
      routeStack("/payment", "post") || routeStack("/services/payment", "post");
  ...
  it("should contain payment feature @app-payment-feature", () => {
    assert(
      typeof app === "function",
      "`app` const has not been created in `app.js`."
    );

    assert(
      typeof getHandleSpy === "function",
      "The payment get route may not exist."
    );
    ...
    assert(
      getRes.render.called,
      "The payment get route may have not been created."
    );
    assert(
      getRes.render.calledWithExactly("payment", sinon.match.object),
      "`res.render` is not being called with the correct arguments."
    );

    assert(
      typeof postHandleSpy === "function",
      "The payment post route may not exist."
    );
    let accounts;
    try {
      ...
      assert(
        accounts !== undefined,
        "Has the `accounts` variable been created in `app.js`?"
      );
    }
    ...
    if (fs.existsSync(path.join(process.cwd(), "src/data.js"))) {
      assert(
        /writeJSON/.test(postHandleOriginal.toString()),
        "The transfer post function does not include a call to `writeJSON`."
      );
    } else {
      assert(
        /accountsJSON/.test(postHandleOriginal.toString()),
        "The payment post function does not include an `accountsJSON` const."
      );
      assert(
      ...
      assert(
        postHandleOriginal.toString().match(/parseInt/).length >= 1,
        "Make sure to use `parseInt`."
      );
    }

    assert(
      postRes.render.called,
      "The payment post route may have not been created."
    );
    assert(
    ...
    assert(
      writeFileSyncStub.firstCall.args[0] ===
    ...
    // console.log(writeFileSyncStub.firstCall.args[2]);
    // assert(
    //   writeFileSyncStub.firstCall.args[2].args[2]
    //     .replace("-", "")
    //     .toLowerCase() === "utf8",
