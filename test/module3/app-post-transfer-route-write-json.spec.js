...
describe("Transfer post route write JSON", () => {
  ...
  before(() => {
    stack =
      routeStack("/transfer", "post") ||
      routeStack("/services/transfer", "post");
  ...
  it("should contain the index route @app-post-transfer-route-write-json", () => {
    ...
    const res = mockRes();
    assert(
      typeof handleSpy === "function",
      "The transfer post route may not exist."
    );
    handleSpy(req, res);
    ...
    assert(
      typeof writeFileSyncStub.firstCall.args[2] !== "undefined",
